...
class PostAdapter constructor(private val onClick: ((id: Int, item: PostModel) -> Unit)? = null) :
    BaseMultiItemQuickAdapter<PostModel, BaseViewHolder>() {

    companion object {
        const val avtKey: Int = 1
    }

    init {
        addItemType(PostModel.POST, R.layout.item_home_post_layout)
        addItemType(PostModel.SHARE, R.layout.item_home_post_share_layout)
    }
    ...
    private fun setUpPostShare(holder: BaseViewHolder, post: PostModel) {
        ...
        BaseApplication.instance?.dataBase?.getReference(ConstantKey.usersRefer)
            ?.child(post.userID!!)?.get()?.addOnSuccessListener {
                ...
                //Xử lý rule private theo bạn
                if (userPost?.privateTypePost != ConstantKey.isPublic) {
                    when {
                        userPost?.privateTypePost == ConstantKey.isOnlyMe && post.userID != myInfo?.uid -> {
                            remove(post)
                        }
                        userPost?.friends?.containsValue(myInfo?.uid) == false && post.userID != myInfo?.uid -> {
                            remove(post)
                        }
                    }
                }
                ...
                avtImg.setOnClick {
                    if (userPost?.uid != userID) {
                        onClick?.invoke(avtKey, post)
                    }
                }
                                        ...
                                        if (likeValue["userID"] == myInfo?.uid) {
                                            ...
                                            holder.setText(
                                                ...
                                                "${post.like?.values?.size
                                                    ?: 0} ${context.getString(R.string.like)}"
                                            )
                                            holder.getView<ImageView>(R.id.likeImg)
                                                .setColorFilter(
                                                    ContextCompat.getColor(
